...
def benchmark(model_path, device="intel:gpu", imgsz=640, rounds=50):
    ...
    for _ in range(5):
        model.predict(source=frame, device=device, imgsz=imgsz, half=True, verbose=False)
    ...
    print(f"  Total Wall Clock: {np.mean(latencies):.2f} ms ({1000/np.mean(latencies):.2f} FPS)")
    ...
    print(f"Results (THROUGHPUT mode):")
    print(f"  Total Wall Clock: {np.mean(latencies_tp):.2f} ms ({1000/np.mean(latencies_tp):.2f} FPS)")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OpenVINO 模型性能测试工具")
    parser.add_argument("--model", type=str, default="models/best_openvino_model", help="模型路径")
    parser.add_argument("--device", type=str, default="intel:gpu", help="推理设备 (cpu, intel:gpu)")
